...
describe('integration: special commands', () => {
  ...
  // Unset CLAUDECODE to allow running ccs outside nested session check
  const env = Object.fromEntries(
    Object.entries(process.env).filter(([k]) => k !== 'CLAUDECODE')
  );

  it('shows version with --version', () => {
    const output = execSync(`node ${ccsPath} --version`, { encoding: 'utf8', env });
    assert(/\d+\.\d+\.\d+/.test(output));
  });

  it('shows version with -v', () => {
    const output = execSync(`node ${ccsPath} -v`, { encoding: 'utf8', env });
    assert(/\d+\.\d+\.\d+/.test(output));
  });
